...
# Edit genesis
sed -i "s/stake/$DENOM/g" $CONFIG_HOME/genesis.json > /dev/null 2>&1
sed -i 's/"inflation": "[^"]*"/"inflation": "0\.0"/g' $CONFIG_HOME/genesis.json > /dev/null 2>&1
sed -i 's/"inflation_rate_change": "[^"]*"/"inflation_rate_change": "0\.0"/g' $CONFIG_HOME/genesis.json > /dev/null 2>&1
sed -i 's/"inflation_min": "[^"]*"/"inflation_min": "0\.0"/g' $CONFIG_HOME/genesis.json > /dev/null 2>&1
sed -i 's/"voting_period": "[^"]*"/"voting_period": "60s"/g' $CONFIG_HOME/genesis.json > /dev/null 2>&1
sed -i 's/"quorum": "[^"]*"/"quorum": "0.000001"/g' $CONFIG_HOME/genesis.json > /dev/null 2>&1

# Edit config.toml to set the block speed to 1s
sed -i 's/^timeout_commit\ =\ .*/timeout_commit\ =\ \"1s\"/g' $CONFIG_HOME/config.toml > /dev/null 2>&1

# Edit app.toml to set the minimum gas price
sed -i "s/^minimum-gas-prices\ =\ .*/minimum-gas-prices\ =\ \"0.0025$GAS_PRICE_DENOM\"/g" $CONFIG_HOME/app.toml > /dev/null 2>&1

# Edit app.toml to enable LCD REST server on port 1317 and REST documentation at http://localhost:1317/swagger/
sed -i 's/^enable\ =\ false/enable\ =\ true/g' $CONFIG_HOME/app.toml > /dev/null 2>&1
sed -i 's/^swagger\ =\ false/swagger\ =\ true/g' $CONFIG_HOME/app.toml > /dev/null 2>&1
